...
class Account(db.Model):
    ...
    def create_user(form: SignUpForm):
        ...
        new_account.password = generate_password_hash(form.password.data, method='sha256')
        try:
            db.session.add(new_account)
            db.session.commit()
        except:
            db.session.rollback()
            email_exists = Account.query.filter_by(email=form.email.data).first()
            username_exists = Account.query.filter_by(username=form.username.data).first()
            if email_exists:
                raise Exception('A user has already been registered using this email.')
            elif username_exists:
                raise Exception('A user has already been registered using this username.')
